function orderOfPresentation(N, K) {
  // 조의 개수 N 3, 발표 순서 K [2,3,1]

  // N은 최대 12입니다.
  // 발표 순서를 만드는 것은 순열(permutation)이므로, 발표 순서의 최대 크기는 12!입니다. 3! -> 경우의수 6개 
  ...
  for (let i = 0; i < K.length; i++) {
    // K의 i번째 조를 변수에 담습니다.
    const kIdx = K[i]-1; //2 , 3 ,  1                              1      2      3
    // 사용했는지 판별하기 위해 isUsed에 체크합니다. (중복이 아니기 때문에) [ true, true, true] 
    isUsed[kIdx] = true;
    // num보다 앞에 올 수 있는 수들의 배열을 복제해서,      1      2
    const preNum = isUsed.slice(0, num); // [false, true] , []
    // 이 중에서 아직 사용되지 않은 수의 개수를 구합니다.
    const unusedCnt = preNum.filter((el) => el === false).length;  //1개 why? 1은 false니까 // 0
    // 아직 사용되지 않은 수, 그 전까지의 모든 경우의 수를 카운트합니다.
    const formerCnt = unusedCnt * factorial(N -i - 1 ); // 1 * factorial(3-1-1) 1* 1 =1 /0
    // order에 추가합니다.
    order = order + formerCnt; //3+0 3
